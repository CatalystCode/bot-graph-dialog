  ...
  generateHeroCard(session, data) {
    ...
    if (data.buttons) {
      ...
      for (let item of data.buttons) {
        switch (item.action) {          
          case "openUrl":
            buttons.push(builder.CardAction.openUrl(session, strformat(item.value, session.dialogData.data), item.label));
            break;
          case "imBack":
            buttons.push(builder.CardAction.imBack(session, strformat(item.value, session.dialogData.data), item.label));
            break;
          case "postBack":
            buttons.push(builder.CardAction.postBack(session, strformat(item.value, session.dialogData.data), item.label));
            break;
        }
      }
